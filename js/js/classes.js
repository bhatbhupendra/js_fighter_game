class Sprite{
    ...
    update(){
        this.draw()
        this.animateFrames() //only animates only when player is not dead
    }
}
...
class Fighter extends Sprite{
    ...
    takeHit(){
        this.health -= 20
        // this.switchSprite('takeHit') //hit amimation // later set in else part whis is same thing
        if(this.health <= 0){ // enemy is dead
    ...
    switchSprite(sprite){
        //overridding all when fighter death
        ...
        switch (sprite) {
            ...
            case 'death':
                if(this.image !== this.sprites.death.image){
                    this.image = this.sprites.death.image            //changing to jump animation
                    this.framesMax = this.sprites.death.framesMax    //setting the maxframe ie 2 for jump
                    this.frameCurrent = 0
                }
                break;
        }
